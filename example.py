        ...
        if model_providers and tts_providers:
            ...
            if available_voices:
                ...
                with st.expander("🎭 Voice Details"):
                    ...
                    st.write(
                        f"**Languages:** {len(selected_voice.languages)} supported"
                    )
                    if len(selected_voice.languages) <= 10:
                        langs = ", ".join(
                            [lang.value for lang in selected_voice.languages]
                        )
            ...
            st.error(
                "No TTS models or voices found. Make sure Google API is configured."
            )
            st.stop()
            ...
            with st.spinner("Generating speech..."):
                try:
                    ...
                    with st.expander("📊 Details"):
                        ...
                        st.write(
                            f"**Sample Rate:** {audio_artifact.sample_rate or sample_rate} Hz"
                        )
